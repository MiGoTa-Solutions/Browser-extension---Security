...
`;document.head.appendChild(h);let m=!1;function d(n,o="info"){const e=document.createElement("div");e.className=`ss-toast ${o}`;const s=o==="success"?"✓":o==="error"?"⚠":"ℹ";e.innerHTML=`<span style="font-size:1.2em">${s}</span><span>${n}</span>`,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("visible")),setTimeout(()=>{e.classList.remove("visible"),setTimeout(()=>e.remove(),400)},3e3)}async function y(){const n=await chrome.storage.local.get(["lockedSites","unlockedExceptions"]),o=n.lockedSites||[],e=n.unlockedExceptions||[],s=window.location.hostname.toLowerCase(),i=o.some(a=>a.is_locked&&s.includes(a.url)),r=e.some(a=>s.includes(a));if(i&&!r)return;const t=document.createElement("button");t.id="ss-float-btn",document.body.appendChild(t),i&&r?(m=!0,t.innerHTML="🔒",t.classList.add("ss-unlocked"),t.title="Site is temporarily unlocked. Click to Re-Lock."):(m=!1,t.innerHTML="白",t.title="SecureShield AI Analysis"),t.onclick=x}async function x(){const o=(await chrome.storage.local.get("auth_token")).auth_token;if(!o||typeof o!="string")return d("Please log in to SecureShield extension.","error");const e=document.getElementById("ss-float-btn");if(m){confirm("Re-lock this website?")&&(e&&(e.innerHTML="..."),await chrome.runtime.sendMessage({type:"RELOCK_SITE",url:window.location.hostname}),d("Site Locked!","success"),setTimeout(()=>window.location.reload(),1e3));return}e&&(e.innerHTML="竢ｳ");try{const s=`Analyze ${window.location.href}. Return valid JSON only: {"description": "string", "pros": ["string"], "cons": ["string"]}`,i=await fetch(`${g}?key=${f}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:s}]}]})});if(!i.ok)throw i.status===429?new Error("Quota Exceeded. Try again later."):new Error(`API Error: ${i.status}`);const r=await i.json();if(!r.candidates||!r.candidates[0]||!r.candidates[0].content)throw new Error("No analysis returned.");let t=r.candidates[0].content.parts[0].text;t=t.replace(/```json/g,"").replace(/```/g,"").trim();const a=JSON.parse(t);k(a,o)}catch(s){console.error("SecureShield AI Error:",s),d(s.message,"error")}finally{e&&(e.innerHTML="白")}}function k(n,o){const e=document.createElement("div");e.className="ss-overlay",e.innerHTML=`
    <div class="ss-popup">
