...
`;document.head.appendChild(g);let u=!1;function c(n,s="info"){const e=document.createElement("div");e.className=`ss-toast ${s}`;const o=s==="success"?"✓":s==="error"?"⚠":"ℹ";e.innerHTML=`<span style="font-size:1.2em">${o}</span><span>${n}</span>`,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("visible")),setTimeout(()=>{e.classList.remove("visible"),setTimeout(()=>e.remove(),400)},3e3)}async function y(){const n=await chrome.storage.local.get(["lockedSites","unlockedExceptions"]),s=n.lockedSites||[],e=n.unlockedExceptions||[],o=window.location.hostname.toLowerCase(),i=s.some(a=>a.is_locked&&o.includes(a.url)),r=e.some(a=>o.includes(a));if(i&&!r)return;const t=document.createElement("button");t.id="ss-float-btn",document.body.appendChild(t),i&&r?(u=!0,t.innerHTML="🔒",t.classList.add("ss-unlocked"),t.title="Site is temporarily unlocked. Click to Re-Lock."):(u=!1,t.innerHTML="🛡️",t.title="SecureShield AI Analysis"),t.onclick=x}async function x(){const s=(await chrome.storage.local.get("auth_token")).auth_token;if(!s||typeof s!="string")return c("Please log in to SecureShield extension.","error");const e=document.getElementById("ss-float-btn");if(u){confirm("Re-lock this website?")&&(e&&(e.innerHTML="..."),await chrome.runtime.sendMessage({type:"RELOCK_SITE",url:window.location.hostname}),c("Site Locked!","success"),setTimeout(()=>window.location.reload(),1e3));return}e&&(e.innerHTML="⏳");try{const o=`Analyze ${window.location.href}. Return valid JSON only: {"description": "string", "pros": ["string"], "cons": ["string"]}`,i=await fetch(`${m}/gemini/analyze`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${s}`},body:JSON.stringify({prompt:o})});if(!i.ok)throw i.status===429?new Error("Quota Exceeded. Try again later."):new Error(`API Error: ${i.status}`);const r=await i.json();if(!r.candidates||!r.candidates[0]||!r.candidates[0].content)throw new Error("No analysis returned.");let t=r.candidates[0].content.parts[0].text;t=t.replace(/```json/g,"").replace(/```/g,"").trim();const a=JSON.parse(t);k(a,s)}catch(o){console.error("SecureShield AI Error:",o),c(o.message,"error")}finally{e&&(e.innerHTML="🛡️")}}function k(n,s){const e=document.createElement("div");e.className="ss-overlay",e.innerHTML=`
  ...
  `,document.body.appendChild(e);const o=document.getElementById("ss-close");o&&(o.onclick=()=>e.remove());const i=document.getElementById("ss-chat");i&&(i.onclick=()=>{e.remove(),w(n.description)});const r=document.getElementById("ss-confirm");r&&(r.onclick=async()=>{try{(await fetch(`${m}/locks`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${s}`},body:JSON.stringify({url:window.location.hostname,name:document.title})})).ok?(chrome.runtime.sendMessage({type:"SYNC_LOCKS"}),c("Site Locked! Refreshing...","success"),setTimeout(()=>window.location.reload(),1e3)):c("Failed to lock site.","error")}catch{c("Network Error.","error")}})}function w(n){const s=document.createElement("div");s.className="ss-overlay",s.innerHTML=`
  ...
  `,document.body.appendChild(s);const e=document.getElementById("ss-chat-box"),o=document.getElementById("ss-chat-input"),i=document.getElementById("ss-send"),r=document.getElementById("ss-close-chat");r&&(r.onclick=()=>s.remove());const t=async()=>{if(!o||!e)return;const a=o.value.trim();if(!a)return;const l=document.createElement("div");l.className="ss-chat-msg ss-msg-user",l.textContent=a,e.appendChild(l),o.value="",e.scrollTop=e.scrollHeight;try{const d=(await chrome.storage.local.get("auth_token")).auth_token,h=await fetch(`${m}/gemini/analyze`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${d}`},body:JSON.stringify({prompt:`Context: ${n}. User Question: ${a}`})}),f=await h.json();if(!h.ok||!f.candidates)throw new Error("AI Error");const p=document.createElement("div");p.className="ss-chat-msg ss-msg-ai",p.textContent=f.candidates[0].content.parts[0].text,e.appendChild(p)}catch{const d=document.createElement("div");d.textContent="Error getting response.",d.style.color="red",e.appendChild(d)}e.scrollTop=e.scrollHeight};i&&(i.onclick=t),o&&o.addEventListener("keypress",a=>{a.key==="Enter"&&t()})}y();
