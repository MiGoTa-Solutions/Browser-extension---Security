const I="http://localhost:4000/api",N="AIzaSyCrBuewLwgc1GhrkZnunYplTnerHfo5rDg",_="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent",m="secure-shield-float-btn",C="secure-shield-style",h="ss-overlay",k="ss-chat-overlay";function $(e){if(!e)return null;try{return e.replace(/^(https?:\/\/)?(www\.)?/,"").split("/")[0].toLowerCase()}catch{return null}}function O(e,t){if(!(e!=null&&e.is_locked))return!1;const n=$(e==null?void 0:e.url);return n?t.includes(n):!1}const T={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"};function w(e){return e.replace(/[&<>"']/g,t=>T[t]??t)}function B(e){return e.replace(/```json/gi,"").replace(/```/g,"").trim()}function j(e){var t;return(t=e.candidates)!=null&&t.length?e.candidates.flatMap(n=>{var o;return((o=n.content)==null?void 0:o.parts)??[]}).map(n=>n.text??"").join(`
`).trim():""}function z(){return typeof chrome<"u"&&!!chrome.runtime}function P(){if(!z())return!0;const e=window.location.protocol;if(["chrome:","edge:","about:","devtools:","file:"].includes(e))return!0;try{const t=chrome.runtime.getURL("");if(window.location.href.startsWith(t))return!0}catch{}return!1}function M(){if(document.getElementById(C))return;const e=document.createElement("style");e.id=C,e.textContent=`
    #${m} {
  ...
  `,document.head.appendChild(e)}async function A(e){const t=await fetch(`${_}?key=${N}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:e}]}]})});if(!t.ok)throw new Error("Gemini API responded with an error");const n=await t.json(),o=j(n);if(!o)throw new Error("Gemini returned an empty response");return o}async function R(e){const t=`You are an expert productivity assistant. Analyze ${e} and reply ONLY with JSON of shape { "description": string, "pros": string[], "cons": string[] } where pros highlight reasons to lock the site.`,n=await A(t),o=B(n);try{const r=JSON.parse(o);return{description:r.description??"AI could not produce a description.",pros:Array.isArray(r.pros)?r.pros:[],cons:Array.isArray(r.cons)?r.cons:[]}}catch(r){throw console.error("Failed to parse Gemini JSON",o,r),new Error("Invalid AI response")}}async function L(){if(P())return;M();const{lockedSites:e}=await chrome.storage.local.get("lockedSites"),t=window.location.hostname.toLowerCase();Array.isArray(e)&&e.some(n=>O(n,t))||H()}function H(){if(document.getElementById(m)||!document.body)return;const e=document.createElement("button");e.id=m,e.type="button",e.setAttribute("aria-label","SecureShield: Lock this site"),e.textContent="🔒",e.addEventListener("click",Y),document.body.appendChild(e)}async function Y(){const{auth_token:e}=await chrome.storage.local.get("auth_token");if(!e){alert("Please log in to SecureShield extension first.");return}const t=document.getElementById(m);t&&(t.textContent="⏳",t.disabled=!0);try{const n=await R(window.location.href);D(n,e)}catch(n){console.error(n),alert("Failed to analyze site.")}finally{t&&(t.textContent="🔒",t.disabled=!1)}}function S(e,t){return e.length?e.map(n=>`<li>${w(n)}</li>`).join(""):`<li>${w(t)}</li>`}function D(e,t){var s,c;const n=document.querySelector(`.${h}[data-overlay="analysis"]`);n==null||n.remove();const o=document.createElement("div");o.className=h,o.dataset.overlay="analysis",o.innerHTML=`
    <div class="ss-popup">
