...
import { syncLocksToChromeStorage } from '../utils/chromeStorage';
...
export function WebAccessLock() {
  ...
  const fetchLocks = useCallback(async () => {
    ...
    try {
      ...
      // Sync to local storage for the content script to use
  ...
  const confirmQuickLock = async () => {
    ...
    try {
      ...
      // Sync immediately
      await syncLocksToChromeStorage([response.lock, ...locks]);
  ...
  const handleCreateLock = async () => {
    ...
    try {
      ...
      const response = await tabLockApi.create(token, payload);
      const newLocks = [response.lock, ...locks];
      setLocks(newLocks);
      await syncLocksToChromeStorage(newLocks);
  ...
  const handlePinAction = async () => {
    ...
    try {
      ...
      const updatedLocks = locks.map((lock) => (lock.id === response.lock.id ? response.lock : lock));
      setLocks(updatedLocks);
      await syncLocksToChromeStorage(updatedLocks);
  ...
  const handleDeleteLock = async () => {
    ...
    try {
      ...
      const updatedLocks = locks.filter((lock) => lock.id !== lockToDelete.id);
      setLocks(updatedLocks);
      await syncLocksToChromeStorage(updatedLocks);
  ...
  // ... (UI Render code remains identical to your original file)
  return (
